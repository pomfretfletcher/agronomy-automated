...
func DoFullStackInteraction(data: Dictionary) -> void:
	if (data["to"] is ShopSlot or data["from"] is ShopSlot) and (data["to"] is InventorySlot or data["from"] is InventorySlot):
...
func FullStackInteraction(data: Dictionary) -> void:
	...
	var used_shop_slot = data["from"] if data["from"] is ShopSlot else data["to"]
	
	var selling = (used_inventory_slot == data["from"])
	var buying = (used_inventory_slot == data["to"])
	
	# Decides order of item movement between slots
	var from_slot: InterfaceSlot; var to_slot: InterfaceSlot
	if selling: from_slot = used_inventory_slot; to_slot = used_shop_slot
	if buying: from_slot = used_shop_slot; to_slot = used_inventory_slot
	...
	if selling:
		...
		if to_slot.is_empty:
			...
			if is_new_item_in_shop:
				...
				RemoveFromShop(to_slot.item_name)
...
func SingleItemInteractionFromInventory(from_slot: InventorySlot) -> void:
	...
	if sent_item in shopslotcomp.current_contents.keys():
		...
		var to_slot: ShopSlot = FindFirstEmptySlot("shop")
...
#	Item bought
func SingleItemInteractionFromShop(from_slot: ShopSlot) -> void:
	var sent_item: String = from_slot.item_name
	var sell_value = GetSellValue(sent_item, 1)
...
# Debug - N/A
func ClearShopSlot(slot: ShopSlot) -> void:
	slot.RemoveItem()
...
# Debug - N/A
func SetupShopSlot(slot: ShopSlot, item_name: String) -> void:
