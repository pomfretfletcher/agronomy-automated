...
# By - Child Classes - UpdateShoppingSlotAmounts, UpdateStorageSlotAmounts
...
func FindHotbarSlotWithItem(item_name: String) -> InterfaceSlot:
	for slot in hotbar_interface.hotbar_slots:
		var cur_slot = slot as InterfaceSlot
		if cur_slot.item_name == item_name:
			return cur_slot
...
func ToggleHotbarInterfaceMode(mode: String) -> void:
	if mode == "open":
		hotbar_interface.being_used_in_interface_interaction = true
		hotbar_interface.full_stack_movement.connect(DoFullStackInteraction)
		hotbar_interface.single_item_movement.connect(DoSingleItemInteraction)
	if mode == "close":
		hotbar_interface.being_used_in_interface_interaction = false
		hotbar_interface.full_stack_movement.disconnect(DoFullStackInteraction)
		hotbar_interface.single_item_movement.disconnect(DoSingleItemInteraction)
...
func UpdateInventoryOrHotbarSlotAmounts(slot: InterfaceSlot, item_name: String) -> void:
	if slot is HotbarSlot:
		UpdateHotbarSlotAmounts(item_name)
	elif slot is InventorySlot:
		UpdateInventorySlotAmounts(item_name)
...
func UpdateHotbarSlotAmounts(item_name: String) -> void:
	var slot = FindHotbarSlotWithItem(item_name)
	if slot:
		slot.UpdateAmountLabel(item_name)
	
	if slot == null:
		print("Error finding " + item_name + " in hotbar slots.")
