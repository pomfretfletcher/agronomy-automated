...
@onready var hotbar_inventory_interaction_component: HotbarInventoryInteractionComponent = get_tree().get_first_node_in_group("hotbarinventoryinteractioncomponent")
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("open_and_close_inventory"):
		if is_opened:
			CloseInterface()
			hotbar_inventory_interaction_component.CloseInterfaces()
		else:
			OpenInterface()
			hotbar_inventory_interaction_component.OpenInterfaces(hotbar_interface)
...
func ApplyLoadedData(loaded_save_data: Dictionary) -> void:
	...
	if loaded_save_data.has("current_contents"): 
		...
		slotcomp.current_contents.clear()
